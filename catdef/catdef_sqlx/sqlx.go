...
func InitSQLx(cfg catcfg.Config) (*Database, error) {
	db, err := sql.Open("postgres", cfg.ConfigDB.GetDSN())
	if err != nil {
		return nil, err
	}

	if cfg.Logger != nil {
		db = sqldblogger.OpenDriver(cfg.ConfigDB.GetDSN(), db.Driver(), logAdapter{cfg.Logger})
	}
