...
import (
	...
	"github.com/surkovvs/gocat/application/zorro"
)
...
func DefineComponent(cpnt any) Component {
	status := zorro.New()
	if _, ok := cpnt.(interfaces.Healthchecker); ok {
		status.SetStatus(healthcheckReady, healthcheckMask)
	}
	if _, ok := cpnt.(interfaces.Initializer); ok {
		status.SetStatus(initReady, initMask)
	}
	if _, ok := cpnt.(interfaces.Runner); ok {
		status.SetStatus(runReady, runMask)
	}
	if _, ok := cpnt.(interfaces.Shutdowner); ok {
...
func (c Component) IsHealthchecker() bool {
	return c.status.GetStatus().Querying(healthcheckMask) != 0
}

func (c Component) Healthchecker() healthcheck {
...
func (c Component) IsInitializer() bool {
	return c.status.GetStatus().Querying(initMask) != 0
}

func (c Component) Initializer() initialize {
	return initialize(c)
}
...
func (c Component) IsRunner() bool {
	return c.status.GetStatus().Querying(runMask) != 0
}

func (c Component) Runner() run {
	return run(c)
}
...
func (c Component) IsShutdowner() bool {
	return c.status.GetStatus().Querying(shutdownMask) != 0
}

func (c Component) Shutdowner() shutdown {
	return shutdown(c)
}
